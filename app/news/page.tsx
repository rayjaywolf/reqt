                ...
                <main className="w-full h-full min-h-[calc(100vh-200px)] ">
                    <div className="min-h-[calc(100vh-200px)] flex flex-col ">
                        <section className="bg-black border-4 border-white p-0 overflow-y-auto flex-1 flex flex-col shadow-[8px_8px_0_0_#000] ">
                            <div className="bg-yellow-400 text-black p-3 border-b-4 border-black">
                                <h1 className="text-3xl font-mono font-black tracking-tight text-center uppercase">
                                    LATEST NEWS
                                </h1>
                            </div>

                            <div className="p-6 flex-1 flex flex-col overflow-y-auto space-y-8 px-16">
                                {/* Featured Article */}
                                {isLoading ? (
                                    <div className="border-4 border-black p-4 bg-black mb-6">
                                        <div className="text-white font-mono text-xl mb-4">LOADING ARTICLES...</div>
                                    ...
                                    <div className="border-4 border-black p-4 bg-black mb-6">
                                        <div className="text-white font-mono text-xl">NO ARTICLES FOUND</div>
                                    </div>
                                )}

                                {/* Category Filter */}
                                <div className="border-4 border-black p-5 bg-white">
                                    <div className="text-black font-black text-2xl mb-3">FILTER BY CATEGORY</div>
                                    <div className="flex flex-wrap gap-3">
                                        {['ALL', ...categories].map((category) => (
                                            <button
                                                key={category}
                                                onClick={() => setSelectedCategory(category === 'ALL' ? null : category)}
                                                className={`px-5 py-2 ${selectedCategory === (category === 'ALL' ? null : category)
                                                    ? 'bg-black text-white'
                                                    : 'bg-[#FF6B00] text-black'
                                                    } border-2 border-black font-mono font-black uppercase text-sm 
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    {filteredArticles.slice(1).map((article, index) => (
                                        ...
                                        >
                                            <div className="relative h-64 w-full border-4 border-black">
                                                <Image
                                            ...
                                            <div className="p-4 border-t-4 border-black">
                                                ...
                                                <h3 className="text-white font-black text-xl mb-2 border-2 border-black bg-black p-2">
                                                    {article.title}
                                                </h3>
                                                <div className="flex justify-between items-center">
                                                    <div className="text-white text-xs bg-black p-2 border-2 border-white">
                                                        🗓 {formatDate(article.date)}
                                                    </div>
                                                    <div className="bg-yellow-400 text-black px-3 py-1 border-2 border-black font-mono font-black text-sm hover:bg-white transition-colors">
                                                        READ MORE →
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                {/* Newsletter Signup */}
                                <div className="border-4 border-black p-5 bg-purple-600 mt-8">
                                    <div className="text-white font-black text-2xl mb-3">GET REKT UPDATES</div>
                                    <div className="flex flex-col md:flex-row gap-3">
                                        <input
                                            type="email"
                                            placeholder="ENTER YOUR EMAIL"
                                            className="px-4 py-3 bg-white text-black border-2 border-black focus:outline-none focus:border-yellow-400 font-mono uppercase text-sm flex-1"
                                        />
                                        <button
                                            className="px-6 py-3 bg-yellow-400 text-black border-2 border-black font-mono font-black uppercase text-sm hover:bg-white hover:border-white transition-all"
                                        >
                                            SUBSCRIBE
                                        </button>
                                    </div>
